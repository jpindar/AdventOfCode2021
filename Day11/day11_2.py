...
def main(filename):
    ...
    add_border()
    # print_data(data)
    # number_of_octopuses does not include the border
    number_of_octopuses = (len(data) - 2) * (len(data[0]) - 2)
    flashes = 0
    steps = 0
    while flashes < number_of_octopuses:
        steps += 1
        [data, flashes] = simulate_step(data)
        # print(f"Step {steps}: {flashes}")
    return steps
...
if __name__ == "__main__":
    ...
    # print(f"result should be 195")
    print()
    filename = "AdventOfCode/Day11/input.txt"
    print(filename)
    result = main(filename)
    print(f"result {result}")
    print(f"result should be 510")
